...
import { Card, CardTitle, KPICard, GlassCard } from '@/components/ui/Card'
...
export default function InvestmentsPage({ data, prefs, addAccount, updateAccounts }: Props) {
  const curveType = prefs.curveType === 'smooth' ? 'monotone' : 'linear'

  const accounts = useMemo(
    () => data.accounts.filter(a => a.category === 'investment' && a.isActive),
    [data.accounts],
  )

  const latest = useMemo(() => getLatestSnapshot(data), [data])

  const disneyConc = useMemo(
    () => (latest ? getDisneyConcentration(latest, data) : 0),
    [latest, data],
  )
  ...
  // Column totals across all investment accounts
  ...
  // Determine if the latest snapshot falls in a month not yet in monthKeys
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <KPICard
          ...
          value={formatCurrency(currentTotal, true)}
      ...
      {accounts.length > 0 && (
        <>
          ...
          <GlassCard className="mb-6" accent="#06b6d4">
            <CardTitle style={{ color: '#06b6d4' }}>Monthly Balances</CardTitle>
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left py-2 pr-4 font-medium text-text-muted whitespace-nowrap min-w-[180px]">
                      Account
                    </th>
                    {monthKeys.map(mk => (
                      <th
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <Card>
              <CardTitle>Investments — Individual</CardTitle>
              <div className="h-72 mt-4">
                <ResponsiveContainer width="100%" height="100%">
          ...
          <Card>
            <CardTitle>Investments — Cumulative</CardTitle>
